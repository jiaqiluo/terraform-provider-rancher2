############################################################
# Purpose:
#   Provision a 3-node RKE2 cluster on AWS using IPv6-only instances,
#   along with a dual-stack bastion host for administrative access.
#   Installs Rancher into the cluster via Helm using a locally
#   downloaded chart tarball (workaround for chart repo IPv6 limits).
#
# Prerequisites:
#   - Existing VPC and subnets: one dual-stack PUBLIC subnet for the bastion,
#     and one IPv6-only PRIVATE subnet for cluster nodes.
#   - Subnets must have IPv6 CIDR associated and routing to an IGW/NAT64 as appropriate.
#
# Usage:
...
kubectl -n kube-system get cm rke2-coredns-rke2-coredns -o json | jq '.data.Corefile |= sub("forward\\s+\\.\\s+/etc/resolv\\.conf"; "forward  . 2001:4860:4860::8888")' | kubectl apply -f -
